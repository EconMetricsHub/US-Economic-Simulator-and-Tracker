...
export async function getLiveDataStatus(env) {
  ...
  try {
    ...
    return {ok:true,status:Number(latest?.count || 0) ? 'ready' : 'empty',d1:true,seriesCount:Number(latest?.count || 0),lastRefresh:latest?.last_refresh || null,lastRun:lastRun || null};
  } catch (err) {
